...
import { Trash2, ShoppingBag, Box, ShieldCheck, AlertTriangle, Heart, Zap, Tag } from 'lucide-react'
...
import { parseVariants, getEffectivePrice, isWholesaleActive as checkWholesaleActive } from '@/lib/pricing'
...
export default function CartPage() {
  ...
  const handleVariantChange = async (
    ...
    const rawProductName = item.name.split(' (')[0]
    
    // Compute new descriptive variant strings cleanly
    await updateItem(itemId, {
      name: `${rawProductName} (${newVariant.name})`,
      retail_price: newVariant.price,
      mrp: newVariant.variant_mrp ?? item.mrp,
      variant_string: newVariant.name,
      wholesale_price: newVariant.wholesale_price ?? item.wholesale_price,
      wholesale_min_qty: newVariant.wholesale_min_qty ?? item.wholesale_min_qty,
    })
            ...
            <div className="lg:col-span-8 flex flex-col">
              ...
              {items.length > 1 && (
                <div className="flex justify-start py-2">
                  <button onClick={() => setShowConfirmClear(true)} className="text-[14px] font-medium text-gray-500 hover:text-red-500 transition-colors cursor-pointer flex items-center gap-1.5 outline-none border-none bg-transparent">
              ...
              <div className="flex flex-col divide-y divide-gray-100">
                {items.map((item, index) => {
                  ...
                  // 🛡️ B2B STRATEGIC MODES GATEKEEPER: Set minimum quantity boundaries based on system flags
                  const isBulkOnly = !item.is_retail_enabled && item.is_wholesale_enabled
                  const minQuantity = isBulkOnly ? (item.wholesale_min_qty || 1) : 1
                  
                  const variants = parseVariants(item.variants)
                  const displayMrp = item.mrp

                  const activeVariant = variants.find(v => v.name === item.variant_string) || null
                  const isWholesaleActive = checkWholesaleActive(item as any, item.quantity, activeVariant)
                  const effectiveUnitPrice = getEffectivePrice(item as any, item.quantity, activeVariant)
                  
                  const baseMrp = item.is_variants_enabled ? (activeVariant?.variant_mrp ?? item.mrp) : item.mrp

                  const discountPct = baseMrp && baseMrp > effectiveUnitPrice
                    ? Math.round(((baseMrp - effectiveUnitPrice) / baseMrp) * 100)
                    : 0
                  return (
                    <div key={item.id} className="flex flex-col gap-4 py-6 text-left">
                      <div className="flex gap-4 md:gap-6 items-start w-full">
                        ...
                        <div className="flex flex-col flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-3 flex-wrap">
                            ...
                            {baseMrp && baseMrp > effectiveUnitPrice && (
                              <span className="text-[20px] sm:text-[22px] text-gray-400 line-through font-normal">
                                {formatCurrency(baseMrp * item.quantity)}
                              </span>
                            )}
                          ...
                          {isWholesaleActive && (
                            <div className="text-[12px] text-emerald-600 mt-0.5 font-semibold">
                              {formatCurrency(effectiveUnitPrice)}/unit (wholesale tier)
                            </div>
                          )}

                          {/* Snapshot-Driven Option Menu Selectors */}
                          {item.is_variants_enabled && variants.length > 0 && (
                            <div className="mt-2.5">
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (selected) {
                                    handleVariantChange(item.id, selected, item)
                                  }
                                }}
                              ...
                              >
                                {variants.map(v => (
                                  <option key={v.name} value={v.name}>
                                    ...
                                    {item.is_wholesale_enabled && v.wholesale_price && v.wholesale_min_qty && ` (${v.wholesale_min_qty}+ → ${formatCurrency(v.wholesale_price)})`}
                                  </option>
                                ))}
                              </select>
                            </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 mt-1 w-full">
                        <div className="flex flex-1 items-center justify-between sm:justify-start gap-3 sm:gap-10 w-full text-[15px] sm:text-[16px] font-medium text-gray-600">
                          <button
                            ...
                            className="flex items-center gap-2 hover:text-pink-500 transition-colors cursor-pointer group py-2 whitespace-nowrap outline-none border-none bg-transparent"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 hover:text-red-500 transition-colors py-2 cursor-pointer outline-none border-none bg-transparent"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 text-gray-900 hover:text-emerald-600 transition-colors whitespace-nowrap py-2 cursor-pointer outline-none border-none bg-transparent"
                          >
            ...
            <div className="lg:col-span-4 w-full text-left">
              <div id="order-summary-section" className="w-full lg:sticky lg:top-24 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4 text-[15px] text-gray-600 font-normal pb-5 border-b border-gray-200">
                  ...
                  <div className="flex justify-between items-start font-normal text-gray-900 text-[18px] pt-4 border-t border-gray-100 mt-2 w-full">
                    ...
                    <div className="flex flex-col items-end text-right flex-1 ml-4">
                      <span className="whitespace-nowrap tracking-tight font-bold">{formatCurrency(finalTotal)}</span>
                ...
                <button
                  ...
                  className="w-full h-14 flex items-center justify-center bg-gray-900 text-white rounded-full font-bold text-[15px] tracking-wide hover:bg-black active:scale-[0.99] transition-colors shadow-none disabled:opacity-50 cursor-pointer border-none outline-none"
                >
                ...
                <div className="flex items-start gap-2 text-[13px] text-gray-500 leading-relaxed font-normal">
                  ...
                  <span>Secure Payments • 100% Organic Essentials • Insulated Fresh Shipping</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
              ...
              <div className="relative z-10 bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm border border-gray-100 animate-zoom-in text-left flex flex-col gap-4">
                ...
                <div className="flex gap-3 pt-4 w-full">
                  <button type="button" onClick={() => setShowConfirmClear(false)} className="flex-1 h-11 text-[14px] font-bold text-gray-700 border border-gray-200 bg-white hover:bg-gray-50 rounded-full transition-colors cursor-pointer outline-none">Cancel</button>
                  <button type="button" onClick={handleClearCart} className="flex-1 h-11 text-[14px] font-bold text-white bg-red-500 hover:bg-red-600 rounded-full transition-colors shadow-sm cursor-pointer outline-none border-none">Clear bag</button>
                </div>
              </div>
            </div>,
            document.body
