...
export default function RecentlyViewed() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        console.log('Loaded recently viewed items:', parsed.length)
  ...
  // 🚀 SKELETON LOADING STATE: Clean, weightless placeholders matching direct layout sizes
  if (!mounted) {
    return (
  ...
  // 🚀 EMPTY STATE: Stripped of bulky box cards, completely open, resting natively on the canvas background
    ...
    return (
      <div className="w-full mt-10 py-12 text-center select-none font-sans antialiased flex flex-col items-center justify-center gap-4">
        ...
        <div className="space-y-1">
          <h3 className="text-[25px] font-normal text-gray-950 capitalise">
            Your history is empty
          </h3>
          <p className="text-[15px] text-gray-400 font-normal max-w-xs mx-auto leading-relaxed capitalise">
  ...
  // 🚀 ACTIVE SHOWCASE VIEW: Beautiful centered un-bolded title row with flat infinite scroll swiper row
  return (
    <div className="w-full mt-10 select-none font-sans antialiased flex flex-col gap-6" suppressHydrationWarning>
      
      {/* Centered, weightless medium text heading track */}
      ...
      {/* Flat open horizontal track without extra enclosing layers */}
      <div 
        className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 scroll-smooth no-scrollbar snap-x snap-mandatory"
      >
